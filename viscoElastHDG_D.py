...
def weaksym(k, h, dt, tend):
    ...
    hatW = VectorFacetFESpace(mesh, order=k+1, dirichlet="bottom|left|right|top")
    fes = FESpace([S, S, S, S, S, S, W, hatW])
    ...
    while t_intermediate < tend:
        ...
        u0.vec[:] = 0.0 
        u0.components[7].Set(exactv, BND)#, dual=True)#BDN --->  Dirichlet Boundary
    ...
    #norm_s = InnerProduct(exactSig - gfsigma, exactSig - gfsigma)
    norm_s  = InnerProduct(a1E*(exactGam - gfsigmaE) - a2E*Trace(exactGam - gfsigmaE)*  Id(mesh.dim), exactGam - gfsigmaE)
    norm_s += InnerProduct(a1D*(exactZe  - omega*gfsigmaV) - a2D*Trace(exactZe  - omega*gfsigmaV)*  Id(mesh.dim), exactZe - omega*gfsigmaV)
...
def collecterrors(k, maxr, dt, tend):
    ...
    for l in range(0, maxr):
        hl = 2**(-(l + 1))
        er_1, er_2 = weaksym(k, hl, dt, tend)
        l2e_s.append(er_1)
        l2e_r.append(er_2)
    return l2e_s, l2e_r
...
er_s, er_u = collecterrors(p, maxlevels, dt, tend)
