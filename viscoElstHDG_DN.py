...
def weaksym(k, h, dt, tend):
    ...
    What = VectorFacetFESpace(mesh, order=k+1, dirichlet="bottom")
    ...
    ft = LinearForm(fes)
    ft += source * v * dx
    ft += (exactSig*n)*vhat.Trace()*ds(definedon=mesh.Boundaries("left|right|top"))
    ...
    while t_intermediate < tend:
        ...
        u0.vec[:] = 0.0 
        u0.components[7].Set(exactv, BND, dual=True)
...
maxlevels = 4 # Levels of mesh refinement
p = 3
dt = (1/16)**(p/2+1) 
tend = 1.5
